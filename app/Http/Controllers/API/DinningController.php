  ...
  public function login(Request $request)
            ...
            if (!$user) {
                ...
                if($user){
                    if (!\Hash::check($password, $user->password)){
                        ...
                        return $this->sendResultJSON("1", "Successfully Login", array("room_id" => 0, "room_number" => "", "occupancy" => 0, "resident_name" => "", "language" => 0, "last_menu_date" => $last_date, "authentication_token" => $user_token,"role" => $role , "form_types" => $formTypes));
                    }
                }else{
                    return $this->sendResultJSON("2", "User not Found");
                }
    ...
    public function getUserData(){
        ...
        if($role == "user"){
            ...
            return $this->sendResultJSON('1', '', array("occupancy" => 0,"resident_name" => "", "language" => 0, "last_menu_date" => $last_date,"role" => $role,'guideline' => setting('site.app_msg'),'guideline_cn'=> setting('site.app_msg_cn') != "" ? setting('site.app_msg_cn') : setting('site.app_msg') , 'form_types' => $formTypes));
        }
    }
    ...
    public function saveForm(Request $request)
    {
        ...
        try {
            ...
            return $this->sendResultJSON("1", "Successfully Submitted", array("submitted_form_id" => $form->id , 'form_link' => Storage::url('public/FormResponses/'.$uniqueFileName)));
        } catch (\Exception $e) {
            return $this->sendResultJSON("0", $e->getMessage());
        }
    }
    ...
    public function getFormDetails(Request $request){
        try {
            ...
            return $this->sendResultJSON("1", "Fetched Form Data Successfully",['form_data' => $data]);
        } catch (\Exception $e) {
            return $this->sendResultJSON("0", $e->getMessage());
        }
    }
    ...
    public function editGeneratedFormResponse(Request $request)  {
        ...
        try {
            ...
            return $this->sendResultJSON("1", "Successfully Submitted", array('new_form_link' => Storage::url('public/FormResponses/'.$uniqueFileName)));
        } catch (\Exception $e) {
            return $this->sendResultJSON("0", $e->getMessage());
        }
    }
    
    public function getGeneratedForms(){

        try{
            $list = FormResponse::orderBy('created_at', 'DESC')->with('formType')->get();
